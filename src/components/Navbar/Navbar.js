...
const Navbar = () => {
    ...
    const menu = [
        ...
        { id: 3, text: 'Our History', to: '/history' },
    ...
    return (
        <header className={changeHeader ? "bg-white fixed z-50 top-0 pl-32 pr-32 left-0 w-full shadow-md transition duration-500" : "bg-white pl-32 pr-32 fixed z-50 top-0 left-0 w-full transition duration-500"}>
            {/* desktop nav  */}
            <nav className="flex items-center mx-0">
                ...
                <div className="hidden md:flex lg:flex space-x-8">
                    <ul className="flex items-center space-x-4">
                        {menu.map(item => (
                            <li key={item.id}>
                                <NavLink exact to={item.to} className="text-gray-600 text-lg poppins" activeClassName="border-b-4 border-green-500 text-green-500">{item.text}</NavLink>
                            </li>
                        ))}
                    </ul>
